...
import datasets
from functions import train, validate, get_is, save_samples, LinearLrDecay, load_params, copy_params, cur_stages, images_IS_by_categories
...
#         gen_avg_param = list(p.cuda().to(f"cuda:{args.gpu}") for p in gen_avg_param)
    ...
    """
    #CODICE PER CALCOLARE L'IS PER CATEGORIA (LABEL) DI IMMAGINI
    test_loader = dataset.test
    load_params(gen_net, gen_avg_param, args, mode="cpu")
    images_IS_by_categories(args, test_loader, None, start_epoch, gen_net, lstm_net, writer_dict)
    exit()
    """
    ...
    for epoch in range(int(start_epoch), int(args.max_epoch)):
        ...
        IS, IS_std = get_inception_score_from_directory(f'/home/d.sorge/eeg_visual_classification/eeg_visual_classification_original/TransGAN-master/training_output_lstm_stl/outputEpoch{epoch}')
